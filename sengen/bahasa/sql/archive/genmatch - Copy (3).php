...
consoleMsg( "$gcnt records inserted from match\n");
...
consoleMsg( "$gcnt records inserted total\n");
...
function writeCat($word, $rwords) {
	...
	consoleMsg( "$cnt vocabcat records written\n");
	return $cnt;
}
...
function insertMatches($lwords, $rwords) {
	...
	foreach ($la as $lword_num => $lword) {
		foreach ($ra as $rword_num => $rword) {
			insertVocabMatch($lword, $glpart, $grpart, $rword);
		}
	}
}
