...
function itGet($label) {
	...
	$data = dereference($label);
	$type = 'list';
	if (count($alabel) == 1 && isset($glabels[$label])) {
	if (count($alabel) == 1 && isset($glabels[$label])) {
		$
	}
	if ($label =
	...
	$layer = array($type, $data, $count, $type);
	...
	$it = array();      // an iterator object is a lifo stack
	$gits[$key] = $it;  // store this iterator object in the global list of iterators


	$it[] = array( 'label' => $slabel, 'index' => 0);

	return $key;  // return the key of the newly created iterator object
}
...
/*
I never use recursive functions.
But in this case, in honor of Daniel Baird, I have done it.
And I am sorry for it.

An it is an array of iterator objects.
Each iter

$gits = array();  // global array of iterator objects

Each object in the $git array has two attributes: $key and $it
$key is a guid
$it is a lifo stack of $layer objects

A $layer object has these attributes:
	$type - values: list, label, function 
	$data - the list, the label, or the function name
	$count - the total count of items in the list or label
	$index - the current item in the data
...
function gitNext($key) {
	...
	// the $it is a lifo stack (array)
	// the count of this array = the depth of recursive nesting
	// $list is now the current item on the stack
